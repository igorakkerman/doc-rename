...
Get-ChildItem -filter *.pdf | ForEach-Object {
    ...
    if (${textContent} -NotMatch "yeebase" -or ${textContent} -NotMatch "Chargebee") {
        Write-Verbose "Not t3n legacy. Ignoring ${filename}"
        Return
    }
    ...
    if ( ${textContent} -cmatch "(?s)Rechnung #— (historical_\d+)") {
    ...
    $newFilename = "${invoiceDate} 01 t3n ${invoiceNumber} ${invoiceAmount}€.pdf"
